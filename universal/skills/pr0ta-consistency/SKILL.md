...
Before creating or choosing consistency resources, call `memory_context_pack` for the scene, character, location, prop, wardrobe, or department scope. Use approved memory as the source of truth for continuity. Candidate memory can inform missing-reference work, but keep it labeled as candidate. After approving a character sheet, Element bundle, Seedance Character, global visual bible, wardrobe rule, prop rule, or set continuity decision, record it with `memory_record_decision` or `memory_record_note`.
...
## Preferred Video Consistency Path

**Seedance 2.5 Omni Reference is the preferred default.** Supply approved identity, wardrobe, set, prop, motion, or audio references and give each reference one explicit job. The comparison below applies only when departing from 2.5 Omni because a shot requires Seedance 2.0 trained-character tokens or Kling's Elements and structured controls.
...
| **Reference capacity** | 15 files by modality ceilings: 9 images + 3 video + 3 audio | Images + text only |
...
| **Audio conditioning** | Audio references for rhythm/content guidance; verify sync per take | No audio input |
| **Character persistence** | Per-character ID, per-project | Per-element, per-project |
| **Best for** | Trained character-token workflows and positional reference-token control | Precise camera control, Motion Brush, or existing Element bundles |

Use Seedance 2.5 Omni for the majority of shots. Switch to Seedance 2.0 only when trained character tokens are required, or to Kling for specific shots requiring Elements, structured camera control, multi-shot limits, or Motion Brush.

## On-Screen Text Needs Its Own Continuity Check

Character Elements and identity tokens do not guarantee typography, but text-capable premium video models can generate or animate text successfully. Select an endpoint with verified typography capability, bind or quote the exact copy, and inspect every frame for spelling, glyph, and layout stability instead of assuming that ordinary character-consistency controls protect it.

If native video typography fails that QC pass, generate a verified still with a premium text-capable image model and animate it deterministically on the timeline. See `pr0ta-prompting` → "Line-Locked Poster", `pr0ta-video` → "On-Screen Text — Use Text-Capable Video Models Deliberately", and `pr0ta-timeline` → "Ken Burns as a Clip Property".
...
10. **Long continuous sequences (30s+):** For a PR0TA-available continuation, feed the previous clip to Seedance 2.0 Omni as `@video1` plus approved character stills and treat the result as reference-guided: inspect and trim every join. MuAPI's stronger provider-native extend path requires the original provider `request_id` and is not currently exposed by PR0TA's unified extend catalog. See `pr0ta-video` → `reference/seedance-omni.md` → "Extension".
...
13. **Location consistency via world models:** Anchor a recurring location to one Marble world or scan as the structure authority and to the same approved location plates as the appearance authority; treat every result as reference-guided and review it against the plates. See `pr0ta-hybrid` -> `reference/world-anchored-references.md`.
