...
3. Edit the frame into the target world with an image edit model from `pr0ta-image`, holding the camera and subject placement and changing only the environment. Nano Banana 2 is the default; use Seedream or GPT Image 2.5 Sunburst when adherence to a supplied location still matters.
