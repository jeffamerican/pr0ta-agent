...
>
> **Preferred defaults:** Image → **Nano Banana 2** (`nano_banana_2`). Video → **Seedance 2.5 Omni Reference** (`muapi/seedance-2.5-omni-reference`). Specialist exceptions include Seedance first/last/Edit/Extend, H3 fixed 2K/source edits, FLUX timed keyframes/Draft promotion, LTX Pro/Fast/A2V, Kling controls, Wan 3.0/Prime, and Seedance 2.0 trained characters. Transcription → **Scribe V2**. Assembly → **post-production timeline**.

**GPT image selection:** GPT Image 2.5 supersedes GPT Image 2 (GPT-02). Prefer Sunburst with `quality: "max"` for premium production images and edits; Flare serves faster iteration. Use GPT Image 2 only if the user expressly requests it. Read `pr0ta-image` and the GPT Image 2.5 prompting reference before generation.
...
2. **Use MCP first when available.** PR0TA skill distributions include the remote MCP connector or its setup details. Prefer MCP tools (`create_project`, `list_projects`, `memory_context_pack`, `memory_search`, `memory_record_decision`, `memory_record_note`, `generation_submit`, `voices_list`, `transcription_start`, `transcription_get`, `music_analyze`, `tasks_get`, `assets_list`, `assets_download`, `audio_analyze`, `audio_meter`, `post_sequence_get`, `post_sequence_save`, `post_render_start`, `post_export_start`, `narration_timeline_get`, `narration_materialize_to_post`, `review_submit_assets`, etc.) over ad hoc REST/curl calls. If PR0TA MCP tools are not callable in the session, run the connect canary below and only use REST fallback with redacted auth after confirming the connector is unavailable.
2b. **Use ProtaFilm|memory before and after agent work.** Before writing prompts, revising story, generating assets, styling, cutting, or making department decisions, call `memory_context_pack` for the role/task/scope and treat it as the cited project source of truth. Use `memory_search` for targeted lookups and `memory_graph` when relationships matter. After accepting creative decisions, notes, selected references, or editorial conclusions, write them back with `memory_record_decision` or `memory_record_note`. Approved memory ranks highest; candidate memory is usable but must remain marked as candidate; rejected and superseded memory must not be used by default.
2c. **Use the designed-world prompt contract for controlled reference images.** Call `agent_chat_orchestrate_prompt` with `prompt_profile: "designed_world_reference_image"` and read `pr0ta-prompting` → `reference/designed-world-reference-image.md`. Prefer a first-class `guidance_package` containing one neutral flat structural pass and one visible-pixel-normalized depth pass, followed by the approved appearance authority and required continuity references. The package is indivisible and receipt-backed. The legacy single semantic-asset-ID plus appearance pair remains supported. Do not stack beauty, normals, outlines, false-colour Asset-ID, or duplicate aligned passes with a first-class package, and do not populate Production Designer reference slots or canon without explicit user approval.
2d. **Bootstrap a missing reference through its owning departments.** For a net-new character, hero prop, or wardrobe design, call `agent_chat_orchestrate_prompt` with `character_reference_design`, `prop_reference_design`, or `wardrobe_reference_design` and read `pr0ta-prompting` → `reference/reference-design-bootstrap.md`. These profiles accept zero references, enforce the complete department-correct chain, and return a candidate generation package. Poll with `tasks_get`; do not call `generation_submit` or record approved canon until the user explicitly accepts the candidate.
2e. **Working from footage or a 3D world? Read `pr0ta-hybrid` first.** Probe the plate, decide the matte, build an angle-matched reference plate, and keep structure and appearance authorities separate before submitting SwitchX or a source-video edit.
...
5. **Every audio-bearing asset must be time-indexed before editing — two paths, no exceptions.** Every asset that carries sound must be passed through the correct time-indexing endpoint before it is eligible for the timeline. There are two paths:
   - **Path A — Speech (TTS narration, dialogue clips, music with vocals, video with dialogue/ambient):** call MCP `transcription_start` (Scribe V2 by default); REST fallback is `POST /api/audio/transcription/start`. Output: word-level timing, speaker IDs, speech-adjacent audio events (breath, laughter, applause, speaker_change).
   - **Path B — Instrumental music (score beds, underscore, any asset with no speech):** call MCP `music_analyze`; REST fallback is `POST /api/v2/projects/{project_id}/music/analyze`. Output: `music_analysis.editorial_anchors`, `downbeat_times`, `beat_times`, `transients[]`, `tempo_bpm`, `beat_confidence`. This is the `whisper_index` analogue for music — Scribe V2 does **not** produce beat or downbeat data and must not be used for this.
...
6. **Ken Burns is a clip property.** On the timeline, set Ken Burns via preset name (`push_in`, `pull_back`, `drift_left`, etc.) or custom params — the platform handles super-resolution, FPS consistency, and duration-aware formulas internally. You never write a zoompan expression. See `pr0ta-timeline` → "Ken Burns as a Clip Property".
7. **Verify alignment before final export.** Use `GET /preview?from=&to=&quality=low` to verify key segments — seconds, not minutes. For narration-timeline productions, also call `GET /narration-timeline/verify` before materializing. Do not ship without a verification pass.
8. **Snapshot before major passes.** On the timeline, create a named snapshot before agent edit passes, user review rounds, or any operation that touches many clips. See `pr0ta-timeline`.
9. **Maintain `assets.json` from the first generation.** Append to the asset ledger immediately after every successful generation — not in a batch at the end. See "Project Structure & Asset Management" below.
10. **Run the editorial discipline.** Read `pr0ta-editorial` before any cut. Five-pass rewrite loop, seven ship criteria. A technically assembled cut is not a finished production.
...
PR0TA (app.pr0ta.com) is an AI-powered creative production platform. This skill drives PR0TA through the **bundled MCP connector first**, then the REST API for unexposed routes or high-volume scripting. It supports image, video, speech, music, transcription, persistent consistency resources, post-production timeline assembly, and project-scoped task polling.
...
|------|-----------|-------|
| Image generation (Nano Banana 2, GPT Image 2.5 Sunburst) | `generation_submit` | `generator=image`, `mode=txt_to_img` |
| Image editing (img-to-img, ref-to-img) | `generation_submit` | `mode=img_to_img`, `mode=ref_to_img`, `mode=edit_img` |
| Video generation (Seedance 2.5 Omni Reference) | `generation_submit` | **Preferred default.** `mode=ref_to_vid`, at least one approved image/video/audio reference, 4–30s |
| Video specialist routes (Seedance 2.5 / Hailuo H3) | `generation_submit` | Use exact first/last, Edit/Extend, fixed-2K, mixed-reference, or focused source-edit routes only when the shot requires them |
| Video specialist routes (Seedance 2.0 / Kling O3/V3) | `generation_submit` | Use Seedance 2.0 for trained character tokens; Kling for Elements, 5–6 explicit cuts, structured camera control, or Motion Brush |
| Video specialist routes (Wan 3.0 / Prime) | `generation_submit` | Use exact T2V, I2V, or R2V model IDs; 2–30s, up to 1080p, default-on `enable_audio`, optional I2V `last_image` |
| Voice browsing and selection | `voices_list` | Search/filter provider voices; use returned `selection` in TTS payloads |
| Audio generation (Gemini Flash TTS) | `generation_submit` | `generator=audio`, `mode=txt_to_speech`; ElevenLabs v3 fallback |
| Sound-effect generation | `generation_submit` | `generator=audio`, `mode=text_to_sound`, `model=eleven_text_to_sound_v2` |
| Music generation (ElevenLabs Music) | `generation_submit` | `generator=music`, `mode=txt_to_music` |
| Speech transcription and word timing | `transcription_start`, `transcription_get` | Start Scribe V2, poll with `tasks_get`, then retrieve word timing |
| Task polling/cancel | `tasks_get`, `tasks_cancel` | Submit tools return task IDs, not finished assets |
| Asset list/upload/download links | `assets_list`, `assets_upload_start`, `assets_upload_finalize`, `assets_download` | Storage events auto-finalize signed uploads; use finalize as a fallback if an asset stays uploading |
...
| Final master export | `post_export_start` | Returns an export task ID; poll and verify the resulting asset |
...
| Music beat analysis | `music_analyze` | Analyze instrumental assets, then poll with `tasks_get` |
| Client review | `review_submit_assets` | Submit assets and get a review link |
| Project memory | `memory_context_pack`, `memory_search`, `memory_graph`, `memory_record_decision`, `memory_record_note` | Cited project context before agent work; record accepted decisions and notes after |
| Net-new character/prop/wardrobe prompt | `agent_chat_orchestrate_prompt` | Use the matching reference-design profile with zero or more image inspirations; poll, review the candidate, then generate only after approval |
| Project creation | `create_project` | Create a project for the authenticated user; no browser setup action is needed |
...
**Default path:** Use the PR0TA MCP connector bundled with this plugin. Start with `list_projects`, choose a `project_id`, then call `prep_production_capabilities` to discover the complete page-to-tool profile. This includes Prep read generation, department and Casting workflows, Memory administration, shotlist/storyboard generation, complete Production Queue operation, and required Scribe V2 indexing. Call `memory_context_pack` before prompt-building or production decisions, use `voices_list` when choosing a TTS voice, call `transcription_start` for speech indexing, poll with `tasks_get`, then use `transcription_get` for word timing. All project-scoped MCP tools require `project_id`.

**Connect canary:** If the host's tool search cannot find `list_projects`, the PR0TA MCP connector is probably not authenticated in the current session. Connect `https://app.pr0ta.com/api/mcp/mcp` through the host's remote MCP settings and complete PR0TA OAuth in the browser. Use the host-specific helper or instructions bundled with the installed distribution when available. Start a fresh session if the host retains its original tool inventory. If the host reports successful OAuth and lists PR0TA tools but the model still cannot call them, treat that as a host tool-admission defect rather than a PR0TA outage or a reason to fall back to PAT/REST/browser automation.
...
| **`pr0ta-video`** | Endpoint-specific Seedance 2.0/2.5, Wan 3.0/Prime, Hailuo H3, FLUX 3, LTX 2.5, and Kling prompting; references, storyboards, source-video Edit/Extend, camera/audio behavior, duration/aspect contracts | **Before generating, editing, or extending video** |
| **`pr0ta-image`** | Image models (Nano Banana 2 default, GPT Image 2.5 Sunburst for prompt adherence / character consistency), resolution constraints, fan-out recipe, image editing modes | **Before generating images** |
...
| **`pr0ta-consistency`** | Kling Elements, Seedance Characters (two training paths), character consistency bundles, global visual bible setup, reference pipeline, multi-modal references | **Before any multi-shot production** |
...
| **`pr0ta-prompting`** | Self-contained prompt writing, controlled designed-world reference contracts, model-specific techniques | **Before writing any generation prompt** |
...
| **`pr0ta-hybrid`** | Beeble SwitchX swap/relight, mattes, reference plates, world-anchored structure references | **Before editing footage or anchoring to a world** |
...
0. **Memory Read** — Call `memory_context_pack` for the role/task/scope before building the cue sheet or prompts. Include scene, character, asset, department, or task intent when known. Carry citations and open conflicts into the plan; do not flatten candidate claims into approved facts.
0b. **Cue Sheet** — Build a structured timing plan before any generation: scenes with target durations, named markers (reveals, impacts, transitions), narration text mapped to time windows, music arc, SFX point events. Write self-contained prompts for every shot. Present the cue sheet to the user for approval. Timing changes are cheap here and expensive after generation. See `pr0ta-sync` and `pr0ta-prompting`.
...
3. **Consistency Setup** — First read existing PR0TA prep context with `production_context_get` when the project has a script, breakdown, casting, department-head looks, props, or set references. Use those approved cast sheets, contact sheets, production-designer looks, propmaster assets, and stylist looks before generating anything new. For any character appearing in more than one generated shot, character consistency is mandatory: generate only missing character/location/prop references with Nano Banana 2 (or GPT Image 2.5 Sunburst for challenging character consistency edits), tag approved references via `PATCH /annotations`, then create Element bundles (Kling) and Character profiles (Seedance). Before generation, read each character's consistency bundle (`GET /characters/{id}/consistency`) for provider-ready payloads. See `pr0ta-consistency`.
4. **Key Frame Generation** — Use `img_to_img` or `ref_to_img` with Element/Character references to produce scene key frames.
5. **Video Generation** — Start with Seedance 2.5 Omni Reference and at least one approved image, video, or audio authority. If no reference exists, normally create or approve a keyframe first. Depart from that route only for a concrete shot requirement: Seedance 2.5 first/last, Edit, or Extend; Seedance 2.0 trained character tokens; Wan 3.0 T2V/I2V/R2V or Prime quality; Hailuo H3 fixed 2K or focused source-video edits; FLUX 3 timed keyframes or Draft/Enhance; LTX 2.5 Pro/Fast or audio-to-video; or Kling Elements, multi-shot, structured camera control, or Motion Brush. Load that endpoint's operation-specific `pr0ta-video` reference before writing its prompt or payload; prompt grammar, tokens, and audio fields are not portable. Treat output audio and audio controls separately: Seedance 2.5 and Hailuo H3 are always audio-bearing; Wan 3.0, FLUX 3, and LTX 2.5 generation default audio on; a missing `sound` field is not evidence of silence.
...
7. **Narration and Dialogue** — Generate narration for segments where the speaker is not visible. For visible-speaker dialogue, use native video dialogue only when the selected endpoint exposes and has verified it; otherwise use a dedicated lip-sync workflow or stage the speaker off camera. Gemini Flash TTS is the default for controlled narration/dialogue audio, with ElevenLabs v3 as the fallback for specific ElevenLabs voices or legacy tag workflows. Use `voices_list` before selecting `voice` or `voice_id`. See `pr0ta-audio`.
8. **Time-index EVERY audio-bearing asset (mandatory gate — two paths).** Before anything can enter the editing phase, route each asset through the right indexing endpoint. Speech-bearing assets (narration, dialogue clips, video with audio) use MCP `transcription_start`; instrumental audio (score beds, underscore, and generated point SFX) uses MCP `music_analyze`. REST fallbacks are `POST /api/audio/transcription/start` and `POST /api/v2/projects/{project_id}/music/analyze`. Video-with-sound can either be transcribed directly (PR0TA extracts audio under the hood) or split first via `POST /api/v2/projects/{project_id}/assets/{asset_id}/extract-audio` for reuse. This is a hard gate: no asset moves to step 10 (timeline) until its indexing task has succeeded. For narration-first productions, use transcription word timing; for music-driven beats, use `music_analysis.downbeat_times` / `editorial_anchors`. See `pr0ta-audio`, `pr0ta-sync`, and `pr0ta-api`.
9. **Point SFX** — Generate through MCP `generation_submit` with `generator=audio`, `mode=text_to_sound`; then call MCP `music_analyze` for the generated asset and poll with `tasks_get`. Place it on the SFX track at its cue-sheet marker only after analysis succeeds.
9b. **Curate Assets and Record Memory** — Tag and annotate assets as they're generated: mark hero takes with `approved`/`hero`, tag rejects with `do_not_use`, set `reference_type` on reference images, add `notes` explaining why each asset matters, and favorite the assets destined for the timeline. Prefer MCP `assets_annotations_update` / `assets_annotations_batch_update`; use `style_world_create`, `style_world_update`, `style_world_assign_scenes`, and `style_world_delete` for focused Style-world changes, or `style_package_save` when persisting the Style page and its global visual bible together. REST fallback: `PATCH /api/assets/{project_name}/annotations`. When a selection, rejection, continuity rule, style rule, or client/director note should guide future agents, also call `memory_record_decision` or `memory_record_note` with the relevant scene/department/asset scope. Future agents and collaborators will rely on these labels and memory records to navigate the asset library. See `pr0ta-editorial` → "Asset Curation" and `pr0ta-api` → "Asset Tagging, Readability Filters, and Timeline Analysis".
...
- **Writing prompts?** Read `pr0ta-prompting`. Self-contained prompts, controlled designed-world flat/depth packages with a backward-compatible two-base path, Line-Locked Poster pattern, model-specific techniques, anti-patterns.
- **Need project context?** Use `memory_context_pack` first and `memory_record_decision` / `memory_record_note` after accepted creative decisions. See `pr0ta-api` → "ProtaFilm|memory MCP Tools".
- **Generating images?** Read `pr0ta-image`. Nano Banana 2 (default), GPT Image 2.5 Sunburst (for prompt adherence / character consistency edits), resolution constraints, fan-out and pick, image editing modes.
- **Generating video?** Read `pr0ta-video`, then the exact model reference: `reference/hailuo-h3.md`, `reference/wan-3.0.md`, `reference/flux-3.md`, `reference/ltx-2.5.md`, `reference/seedance-2.5.md`, `reference/seedance-omni.md`, or `reference/kling-prompting.md`. Their token grammars and audio/reference fields are not interchangeable.
...
- **Downloading assets?** Read `pr0ta-downloading`. Curl-via-subprocess Cloudflare handling, authenticated MCP handoffs, bearer auth, and scoped delivery URLs.
- **Working from live-action plates or a 3D world?** Read `pr0ta-hybrid`. Beeble SwitchX background swap and relight, matte and alpha sourcing, reference-plate authoring, structure-versus-appearance authority, collider structure passes, world-only previs guides, pose-bearing captures, and camera-take import.
