...
**Connect canary:** If tool search cannot find `list_projects` (for example, through ToolSearch in Claude Code), the PR0TA MCP connector is probably not authenticated in this Claude session yet. The connector is bundled at the plugin root (`.mcp.json`); if it is missing from the session, register it with the bundled helper when available:

```bash
scripts/connect-mcp.sh   # relative to the plugin root
```

If the helper path is not available in the current install, register it manually:

```bash
claude mcp add --transport http pr0ta https://app.pr0ta.com/api/mcp/mcp
```

Then ask the user to run `/mcp` in Claude Code, select **pr0ta**, and complete the browser OAuth approval. After the browser login completes, retry tool search for `list_projects` (restart the session if the tools still do not appear). Do not treat missing tools as a PR0TA API outage until this OAuth connect step has completed. If `/mcp` reports `Auth: OAuth` and lists PR0TA tools but the model still says they are unavailable, treat that as a host/tool-admission defect rather than a PR0TA outage or a reason to fall back to PAT/REST/browser automation.
