...
- one approved chronological storyboard reference sheet showing the major visual beats in strict order
...
Do not submit several independent storyboard or shot-frame images to the final generation. Generate variations during Prep, approve one sheet, and exclude the others so the temporal authority is unambiguous.
...
This route is a prompt compiler, not a general agent loop. It freezes the brief,
resolved references, target, and authority/memory packet; disables department
tools; runs one structured call per role; and permits only one repair or one
retry of the same Settings-configured runtime. Stylist and Propmaster run in
parallel after their shared prerequisites. A `prompt_assessment` is returned for explicit review
rather than triggering recursive clarification calls. Exhausted attempts omit a
resume token; resume only when the failure explicitly supplies one.

The role boundary is:

1. Director chooses the generatable shot count, pacing, and camera structure from the frozen
   target model, duration, story, style, and reference authority. Prompt-directed Seedance plans
   may use a sustained take or rapid cuts; do not impose a generic one-to-three-shot limit.
2. Stylist contributes only visible wardrobe, hair, and makeup facts, or `not_applicable`.
3. Propmaster contributes only visible prop identity and state facts, or `not_applicable`.
4. Storyboarder selects or excludes every submitted reference and binds approved character aliases and reference tokens to the exact shots where they appear.
5. Cinematographer renders and validates the final compact prompt.

Keep every bound character/reference token inside its active `SHOT` block; a token glossary at the top does not establish shot-level control. Keep action, camera, performance, and audio detail proportional to each shot's duration: short cuts stay focused, while a longer take may carry a coherent progression. Separate shots with `HARD CUT`. Do not request instantaneous, one-frame transformations from a video model; frame-exact transitions belong on the Editor timeline.

Casting is the authority for active Omni character resources. Stylist and Propmaster may recommend the correct approved references but cannot invent provider tokens. The generation receipt exposes all validated inline tokens in `omni_character_plan`; PR0TA retains no more than three in the separate structured `character_ids` field.
...
Critical instruction: animate the storyboard sheet in [storyboard_sheet_token] in strict chronological panel order. Start with Panel 1, pass through every panel exactly once in sequence, and end on the final panel state.

Maintain the global visual bible from @image1 throughout. Follow the storyboard sheet for composition and blocking, but keep the motion cinematic and continuous. Preserve character identity, set geometry, wardrobe, prop design, color palette, and lens texture. The output contains only the approved characters, props, locations, logos, and text named above.
```
...
- Text-heavy cards may use a premium video endpoint with verified typography capability, but this Seedance storyboard path does not itself guarantee exact copy. Inspect every frame; use a verified still and deterministic timeline animation when the selected result drifts.
