...
```
assets_annotations_update
assets_annotations_batch_update
```

Use the batch tool for style packages and other multi-reference Prep updates. Use
`style_package_save` when the same operation must also persist the Style page's
top-level `styleReferences` metadata.
