...
- include `review_round_id`, `submission_id`, and the asset identity snapshotted when a review decision was recorded;
- include their referenced submission records in top-level `review_submissions`; and
- may supply an actor-attributed `approved` or `approved_with_notes` event ID directly as `memory_record_decision.user_confirmation_ref` only while it remains the active decision after the latest publish marker.

Once any review decision exists, the submission's asset cannot be replaced or cleared. A later `changes_requested` decision or republish marker invalidates an earlier approval for memory confirmation.
