...
```bash
./scripts/connect_pr0ta_mcp.sh
```

The helper configures file-backed MCP OAuth storage so Codex CLI and Codex Desktop share the same credential on macOS, and enables the complete Prep/Production tool profile in Codex's MCP config. Codex should open or print a PR0TA authorization URL. If it prints a URL, present that URL to the user and ask them to complete the browser approval. After the browser login completes, fully quit and reopen Codex, start a new task, and retry `tool_search` for `list_projects`. Resuming an existing task preserves its original tool inventory. If `/mcp` reports `Auth: OAuth` and lists PR0TA tools but the model still says they are unavailable, treat that as a Codex host/tool-admission defect rather than a PR0TA outage or a reason to fall back to PAT/REST/browser automation.
