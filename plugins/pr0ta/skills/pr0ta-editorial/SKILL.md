...
Before an edit pass, call `memory_context_pack` with `agent_role: "editor"` and a task intent such as `"edit_notes"`, `"producer_review"`, or `"client_review_fix"`. Treat approved memory as the project source of truth, carry candidate memory only as labeled candidate guidance, and stop to resolve conflicts that affect story, continuity, client notes, or asset usage. After the pass, record durable editorial conclusions, accepted client/director notes, rejected takes, and continuity constraints with `memory_record_decision` or `memory_record_note`.
...
- **Missing title card** — the section needs a breath and a label. Use a verified premium image or video model for the typography. Native animated text is valid when the endpoint supports it and every frame passes copy/legibility QC; otherwise add a verified still to the post-production timeline with a hold or Ken Burns preset.
...
2. **Build the shot checklist.** For each note, record timestamp, frame/timecode, note body, transcript phrase, active clip ID, asset ID, source name, and action. Use `GET /timeline/clips?sequence_id=` and select clips whose `start <= time < end`, preferring `video` or `title` entries with `trackMuted: false`; if none overlaps, record the nearest clip explicitly.
