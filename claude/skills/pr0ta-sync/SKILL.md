...
Before building a cue sheet, timing plan, narration-first plan, or montage strategy for an existing project, call `memory_context_pack` with the relevant scene/department/task scope. Carry approved decisions, candidate claims, conflicts, open questions, and continuity constraints into the plan with citations. After the user accepts timing, cue, or sync decisions, record them with `memory_record_decision` or `memory_record_note`.
...
4. **Point SFX** — Generate short sound effects for specific hit points, then include each generated SFX in the indexing pass before it can be placed.
5. **Time-index every audio-bearing asset (hard gate — two paths).** Every speech-bearing asset (TTS narration, dialogue clips, music with vocals, video with `sound: "on"`) must be transcribed with Scribe V2 via MCP `transcription_start`; REST fallback is `POST /api/audio/transcription/start`. Every instrumental audio asset (score bed, underscore, stinger, point SFX, foley) must go through MCP `music_analyze` (REST fallback: `POST /api/v2/projects/{project_id}/music/analyze`) for beat/downbeat/transient anchors. Scribe V2 does not detect musical beats; the music analysis endpoint does not transcribe words. Pick the right path per asset and wait for the indexing task to succeed. This is non-negotiable — see `pr0ta-audio` → "Mandatory Time-Indexing Rule (Two Paths)" and "Music Analysis API" for the full policy.
6. **Build on the Timeline** — Add clips only after their required transcription or music analysis has succeeded; then set Ken Burns, configure audio mix, preview, snapshot, and hand off.
...
2. **Transcribe** — call MCP `transcription_start` with `asset_id` and `project_id`, poll with `tasks_get`, then retrieve word timing with `transcription_get`. REST fallback: `POST /api/audio/transcription/start`. **Scribe V2 is the preferred transcription provider** — it returns speaker IDs, audio events (laughter, applause), and per-word `event_type`. Whisper is still available as a fallback but is not recommended for narration-timeline work. The transcript layer auto-populates in the narration timeline with word-level timestamps, sentence boundaries, and paragraph boundaries.
...
2. Generate all video clips with target durations from the cue sheet. **Seedance 2.5 Omni Reference is the preferred production path**; attach at least one approved image, video, or audio authority per shot. Choose Seedance 2.0, H3, Kling, or another Seedance 2.5 modality only for a concrete specialist requirement described in `pr0ta-video`. Video generation can take up to 20 minutes on busy days — the reliability contract polls for up to 1200s.
...
**Voice selection:** Gemini Flash TTS is the default and accepts a Gemini `voice` plus `style_instructions`. Use MCP `voices_list` before choosing a voice when the user has not provided an exact voice, when searching by provider/style/language, or when using an ElevenLabs fallback voice. Copy the selected voice's `selection` fields into the TTS request. REST fallback: `GET /api/v2/projects/{project_id}/voices/browser`. See the `pr0ta-audio` skill for the voice browser workflow.
...
Generate each SFX through the dedicated text-to-sound audio path with a short duration:
...
Or for more realistic/foley-style effects, describe them precisely. Keep durations short (1–5 seconds). After each generation, call `music_analyze` with its asset ID and wait for `tasks_get` to report success. Only then add it to an SFX audio track on the post-production timeline aligned to its marker timestamp.
