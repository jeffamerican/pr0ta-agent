...
>
> **Use MCP first.** When the PR0TA MCP connector is available, read/save full sequences with `post_sequence_get` and `post_sequence_save`, start previews with `post_render_start`, start locked final masters with `post_export_start`, and use REST timeline routes only for fine-grained clip/track/editorial primitives that are not exposed through MCP yet.
>
...
Before timeline assembly or a major edit pass, call `memory_context_pack` with `agent_role: "editor"` and the active sequence/scene/task scope. Use approved memory for continuity, client notes, style, pacing, and asset decisions; surface conflicts before mutating the timeline. After saving a meaningful edit decision, accepted fix, timeline snapshot rationale, or final review conclusion, record it with `memory_record_decision` or `memory_record_note`.
...
**Use `POST /sequences` followed by a full `POST /timeline?sequence_id={new_id}` for clean sequence rebuilds.** Sequence creation honors `name`, `sequence_id`, and `sequence` dimensions and starts with empty default tracks; it does not copy another sequence or accept custom tracks/audio mix. Read the source sequence first when settings must be preserved, then construct and save the complete new payload deliberately. Always record the `sequence_id` in scripts, ledgers, render/export calls, and review submissions so you do not render a stale default sequence.
...
Then replace that new empty sequence with the complete intended payload:

```http
POST /api/post-production/{project_id}/timeline?sequence_id=review_v3_clean
Content-Type: application/json
```
...
To preserve settings, audio mix, or track definitions, read the source with `post_sequence_get` or `GET /timeline?sequence_id={source_id}`, remove unwanted clips from the client-side payload, change its name/metadata, and save it to the new `sequence_id`. Use `POST /sequences/{sequence_id}/duplicate` when you want a full copy with all clips intact.
