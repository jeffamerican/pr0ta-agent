...
6. With a configured host receiver, use `tasks_subscribe`, bind `completion_subscription_id` on generation requests (or `tasks_watch` for existing tasks), and acknowledge pickup with `tasks_acknowledge`. Read `reference/task-completion.md` before push delivery or owner email fallback, including when a status read picks up a subscribed result before its notification. Use the project ledger to handle later receipts without repeating assessment or routine status replies. Otherwise poll with `tasks_get`; cancel stuck work with `tasks_cancel`. Call `agent_chat_resume` only when the failure actually includes `error.details.retry_token`; exhausted provider or repair attempts intentionally omit it.
